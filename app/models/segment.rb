class Segment < ActiveRecord::Base
  ...
  scope :has_speed, -> {where(roadtype: [1,2,3,4,6,7,8])}
  ...
  scope :wrong_speed, -> {has_speed.joins(:street).where("vw_streets.name !~* '^[[:alpha:]]{2,3}-[[:digit:]]{3}.*' and ((fwddirection and fwdmaxspeed > 70) or (revdirection and revmaxspeed > 70))")}
  ...
  scope :low_lock, -> {where('roadtype in (3,6,7) and (lock is null or lock < 3)')}
  scope :named_parking, -> {joins(:street).where('roadtype = 20 and not vw_streets.isempty')}
  scope :dirt_street, -> {where('roadtype <> 8 and (flags & 16) = 16')}
