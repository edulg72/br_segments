    ...
    while latIni > latSul do
      ...
      begin
        ...
        json['segments']['objects'].each do |s|
          ...
          @segments[s['id']] = "#{s['id']},#{longitude},#{latitude},#{s['roadType']},#{s['level']},#{(s['lockRank'].nil? ? '' : s['lockRank'] + 1)},#{(s['updatedOn'].nil? ? s['createdBy'] : s['updatedBy'])},#{(s['updatedOn'].nil? ? Time.at(s['createdOn']/1000) : Time.at(s['updatedOn']/1000))},#{s['primaryStreetID']},#{s['length']},#{((s['fwdDirection'] and not json['connections'].has_key?(s['id'].to_s + 'r')) or (s['revDirection'] and not json['connections'].has_key?(s['id'].to_s + 'f'))) ? 'TRUE' : 'FALSE' },#{s['fwdDirection']},#{s['revDirection']},#{s['fwdMaxSpeed']},#{s['revMaxSpeed']},#{(s['junctionID'].nil? ? 'FALSE' : 'TRUE')},#{s['streetIDs'].size > 0},#{s['validated']},#{s['fwdToll'] or s['revToll']},#{s['flags']},\"SRID=4326;LINESTRING(#{nodes.join(',')})\"\n" if not @segments.has_key?(s['id'])
        end
