...
places = db.exec("select id, name, latitude, longitude from vw_places where city_id in (select gid from cities_shapes where state_id = (select cd_geocuf from states_shapes where abbreviation = '#{State}')) and (not residential or residential is null) and approved and lock < #{Lock}")
